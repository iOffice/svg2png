...
function convert(url, options) {
    ...
    page.onLoadFinished = function (status) {
        ...
        /*
         * We want to wati until the standard deviation of the previous 100 FPS checks
         * is less than 1.5, to ensure the svg has rendered properly. The render rate
         * tends to fluctuate while rendering the svg, and level out when it is finished.
        */
        page.evaluate(function() {
            ...
            function calcFPS(now) {
                ...
                fpsList.push(fps);
                var std = stdev(fpsList.slice(fpsList.length - 100, fpsList.length));
                if (std <= 1.5) {
                    console.log("done");
                    return;
                }
            ...
            requestAnimationFrame(function() { console.log("done"); });
        });
