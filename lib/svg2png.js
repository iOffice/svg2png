...
module.exports = async (source, options) => {
    try {
        ...
        const convertPromise = convertBuffer(id, source, options);

        return await Promise.race([convertPromise, timeout]);
    } catch (e) {
        throw e;
    }
};

async function convertBuffer(id, source, options) {
    const output = await pool.use(async (browser) => {
        try {
            ...
            if (page) {
                try {
                    ...
                    console.log("Page close error:", err);
                }
            }
        }
    })
